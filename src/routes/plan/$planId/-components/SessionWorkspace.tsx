...
export default function SessionWorkspace({
	...
	const [selectedSnapshotSessions, setSelectedSnapshotSessions] =
		useState<string[]>(endedSessionIds);
	...
	useEffect(() => {
		if (
			focusedSessionId &&
			sessions.some((bundle) => bundle.session.id === focusedSessionId)
		) {
			return;
		}
		setFocusedSessionId(
	...
	const contextEndpoint = captureSession
		? toAbsoluteUrl(
				appOrigin,
				`/api/sessions/${captureSession.session.id}/context`,
			)
		: "";
	...
	async function handleEndSession() {
		...
		try {
			...
			await router.invalidate();
			setActionNotice(
				"Session ended. It can now be included in drafts and handoffs.",
			);
		} catch (error) {
	...
	async function handleGenerateDraftRevision() {
		...
		try {
			...
			if (draft.content.trim() === latestRevision.content.trim()) {
				throw new Error(
					"The selected sessions did not produce any new plan changes.",
				);
			}
			...
			setActionError(
				getActionError(error, "Failed to generate draft revision"),
			);
		} finally {
			setBusyAction(null);
		}
	...
	async function handleCopy(label: string, value: string) {
		...
		try {
			...
			setActionError(
				getActionError(error, `Failed to copy ${label.toLowerCase()}`),
			);
		}
	}
	...
	return (
		...
		>
			<Accordion
				type="single"
				collapsible
				defaultValue={activeSession ? "workspace" : undefined}
			>
				<AccordionItem value="workspace" className="border-b-0">
					<AccordionTrigger className="flex w-full items-center justify-between gap-3 rounded-2xl px-5 py-4 text-left transition hover:no-underline hover:bg-[var(--surface)]">
						<div className="flex items-center gap-3">
							<div>
								...
								<p className="mt-0.5 text-sm text-[var(--sea-ink-soft)] [[data-state=open]_&]:hidden">
					...
					<AccordionContent className="px-5 pb-5">
						{actionError && (
							<Alert
								variant="destructive"
								className="mb-4 rounded-xl border-red-200 bg-red-50 text-red-700 dark:border-red-800 dark:bg-red-950/30 dark:text-red-400"
							>
								{actionError}
							</Alert>
						)}
						...
						{actionNotice && (
							<Alert className="mb-4 rounded-xl border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300">
								{actionNotice}
							</Alert>
						)}

						{copyState && (
							<Alert className="mb-4 rounded-xl border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-800 dark:bg-sky-950/30 dark:text-sky-300">
								Copied {copyState}.
							</Alert>
						)}
									...
									<div className="space-y-3">
										...
										>
											<div className="space-y-3">
												{sessions.length === 0 ? (
													...
													sessions.map((bundle) => {
														const isFocused =
															focusedSession?.session.id === bundle.session.id;
														return (
															<button
																key={bundle.session.id}
																type="button"
																onClick={() =>
																	setFocusedSessionId(bundle.session.id)
																}
															...
															>
																<div className="mb-2 flex items-center justify-between gap-2">
																	<div>
																		...
																		<p className="text-xs text-[var(--sea-ink-soft)]">
																			{providerLabel(
																				bundle.session.meetingProvider,
																			)}{" "}
																			·{" "}
																...
																<div className="flex flex-wrap gap-2 text-xs text-[var(--sea-ink-soft)]">
																	...
																	<span>
																		{bundle.contextEvents.length} context events
																	</span>
																	<span>
																		{bundle.attentionItems.length} nudges
																	</span>
																</div>
															</button>
														);
										...
										>
											<p className="text-sm text-[var(--sea-ink-soft)]">
												...
												evidence. The public URL is shareable; the callback
												token stays private inside this workspace.
											</p>
										</WorkspacePanel>
									</div>
								</div>
								...
								<div className="grid gap-4 lg:grid-cols-[minmax(260px,0.78fr)_minmax(0,1.22fr)]">
									...
									<div className="space-y-4">
										...
										>
											{captureSession ? (
													...
													<div className="rounded-xl border border-[var(--line)] bg-white/70 p-3 dark:bg-black/10">
														<div className="mb-2 flex flex-wrap items-center justify-between gap-2">
															<div>
																...
																<p className="text-xs text-[var(--sea-ink-soft)]">
																	Drag the link to your bookmarks bar, then
																	click it on GitHub or any page exposing line
																	anchors.
																</p>
															</div>
															<Button
																...
																variant="outline"
																onClick={() =>
																	handleCopy("bookmarklet", bookmarkletHref)
																}
																className="rounded-full"
															>
												...
												<div className="rounded-xl border border-dashed border-[var(--line)] px-4 py-6 text-center text-sm text-[var(--sea-ink-soft)]">
													Start a session first to unlock capture endpoints and
													the bookmarklet.
												</div>
											)}
										</WorkspacePanel>
										...
										<WorkspacePanel
											...
											contentClassName="space-y-3"
											actions={
												<Sparkles className="h-4 w-4 text-[var(--lagoon)]" />
											}
										>
											<div className="grid gap-3 xl:grid-cols-[160px_160px_minmax(0,1fr)_auto]">
												...
												<input
													...
													onChange={(event) =>
														setAttentionSummary(event.target.value)
													}
											...
											>
												{showAdminCaptureTools ? (
															...
															<div className="space-y-3">
																...
																<div className="grid grid-cols-2 gap-2">
																	<input
																		...
																		value={contextRepo}
																		onChange={(event) =>
																			setContextRepo(event.target.value)
																		}
																...
																<Textarea
																	...
																	onChange={(event) =>
																		setContextPayload(event.target.value)
																	}
													...
													<div className="rounded-xl border border-dashed border-[var(--line)] px-4 py-4 text-sm text-[var(--sea-ink-soft)]">
														Manual transcript and context entry are hidden until
														you open the admin tools.
													</div>
												)}
											</WorkspacePanel>
										)}
							...
							<TabsContent value="handoff" className="space-y-4">
								<div className="grid gap-4 lg:grid-cols-[minmax(280px,0.92fr)_minmax(0,1.08fr)]">
									...
									>
										...
										<div className="space-y-2">
											...
											{endedSessions.length === 0 ? (
												<p className="rounded-xl border border-dashed border-[var(--line)] px-3 py-4 text-sm text-[var(--sea-ink-soft)]">
													No ended sessions yet. You can still draft or publish
													a revision-only handoff, but it will have no captured
													evidence.
												</p>
											) : (
															...
															>
																...
																<span>
																	...
																	<span className="text-xs text-[var(--sea-ink-soft)]">
																		{bundle.transcriptChunks.length} transcript
																		chunks, {bundle.contextEvents.length}{" "}
																		context events,{" "}
																		{bundle.attentionItems.length} nudges
																	</span>
																</span>
										...
										{!isAuthor && (
											<p className="text-xs text-[var(--sea-ink-soft)]">
												Only the plan author can create revisions or publish
												handoffs.
											</p>
										)}
									</WorkspacePanel>
